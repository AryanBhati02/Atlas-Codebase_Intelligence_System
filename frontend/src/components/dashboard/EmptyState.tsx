...
import { GitBranch, Clock, ExternalLink, Loader2 } from "lucide-react";
import { useAppStore } from "../../store/appStore";
import type { RecentRepo } from "../../store/appStore";
import { ingestGitHub } from "../../api/ingest";

export function EmptyState() {
    const { recentRepos, setShowIngestModal, setSession, setError } = useAppStore();
    ...
    const handleRecentClick = async (repo: RecentRepo) => {
        if (repo.sourceType === "github" && repo.url) {
            ...
            setError(null);
            try {
                const data = await ingestGitHub(repo.url);
                setSession(data);
            } catch (err: unknown) {
