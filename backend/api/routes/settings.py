"""
Settings API routes — AI provider key management, status, and cache control.

Endpoints:
  GET  /api/settings          → current config (keys masked, model names, status)
  POST /api/settings/keys     → validate key, save to .env, hot-reload
  POST /api/settings/test     → test a provider with a live prompt
  GET  /api/settings/status   → live provider availability
  POST /api/settings/prefer   → set local vs API preference
  POST /api/settings/clear-cache → clear AI response cache
"""
...
def _get_provider_status_label(provider: str) -> str:
    """Determine the current status label for a provider."""
    if provider == "ollama":
        try:
...
def _count_cache_entries() -> tuple[int, float]:
    """Count total AI cache entries and size across all sessions."""
    total_entries = 0
    total_bytes = 0
...
def _clear_session_cache(session_dir: Path) -> int:
    """Clear the AI cache for a single session. Returns entries cleared."""
    cache_db = session_dir / "ai_cache.db"
...
def _determine_active_provider() -> str:
    """Determine which provider would currently be used first."""
    if get_prefer_local():
        return "ollama"
...
async def get_settings():
    """Return current configuration with masked keys, model names, and stats."""
...
async def update_key(request: KeyUpdateRequest):
    """Validate an API key, save to .env, and hot-reload."""
...
async def test_provider_endpoint(request: TestProviderRequest):
    """Test a provider with a live prompt and return latency."""
    provider = request.provider.lower()
...
async def get_status():
    ...
    Does NOT send test prompts — uses cached knowledge.
    For live testing, use POST /test.
    """
...
async def set_preference(request: PreferenceRequest):
    """Set whether to prefer local Ollama or API providers."""
...
async def clear_cache(request: ClearCacheRequest):
    """Clear AI response cache for a specific session or all sessions."""
    total_cleared = 0
